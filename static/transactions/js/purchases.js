...
function fetchPurchaseInformation() {
    ...
    if (activeModal.attr('id') === 'popup-add') {
        ...
        purchaseInfo.oldStuffDetails = {};

        purchaseInfo.oldStuffDetails.oldStuffValue = $('#old-stuff-value').val();
        purchaseInfo.oldStuffDetails.oldStuffNote = $('#old-stuff-note').val();
...
$(document).ready(function() {
    ...
    $(document).on('click', '#calculate-button-add', function () {
        ...
        const oldStuffValue = parseFloat(purchaseInfo.oldStuffDetails.oldStuffValue || 0);
    ...
    $(document).on('click', '.view-purchase-bill', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const oldStuffNote = purchaseInfo?.billno__old_stuff_note || '';
                const oldStuffValue = parseFloat(purchaseInfo?.billno__old_stuff_value || 0);
    ...
    function performSearch(page) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                swal("Error", "Error searching purchases: " + error, "error");
            }
        });
    }

    // Event handler to delete purchase bills
    ...
    $('#deleteButton').click(function(event) {
        ...
        swal({
            title: "Are you sure?",
            text: "This will permanently delete the matching purchase bills.",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        }).then((willDelete) => {
            if (willDelete) {
                $.ajax({
                    url: '/transactions/delete-purchase/',
                    type: 'POST',
                    headers: {
                        'X-CSRFToken': csrftoken
                    },
                    ...
                    error: function(xhr, status, error) {
                        swal("Error", "Error deleting purchase bills: " + error, "error");
                    }
                });
            }
        });
